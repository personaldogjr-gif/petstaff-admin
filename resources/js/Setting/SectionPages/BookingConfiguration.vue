<template>
  <form @submit="formSubmit">
    ...
    <CardTitle title="Configurações de Reserva" icon="fa-solid fa-paper-plane"></CardTitle>

    <div class="row mb-4">
        <div class="col-md-6">
          <InputField  :label="`${$t('setting_booking_page.lbl_pet_daycare_amount')} (${CURRENCY_SYMBOL})`"  placeholder="Valor da creche" v-model="pet_daycare_amount" :errorMessage="errors.pet_daycare_amount"></InputField>
          <span>{{ $t('setting_booking_page.daycare_message') }}</span>

        </div>
        <div class="col-md-6">
          <InputField  :label="`${$t('setting_booking_page.lbl_pet_boarding_amount')} (${CURRENCY_SYMBOL})`" placeholder="Valor da hospedagem" v-model="pet_boarding_amount" :errorMessage="errors.pet_boarding_amount"></InputField>
