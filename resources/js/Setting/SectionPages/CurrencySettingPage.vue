<template>
  <CardTitle title="Configurações de Moeda" icon="fa fa-dollar fa-lg mr-2">
      ...
      <template v-if="tableList !== null && tableList.length !== 0">
        <tbody>
          <tr v-for="(currency, index) in tableList" :key="index">
            ...
            <th>
              <span v-if="currency.is_primary" class="badge bg-soft-success py-2 px-3">Padrão</span>
            ...
            <th>
              ...
              <button type="button" class="fs-4 text-danger border-0 bg-transparent" @click="destroyData(currency.id, 'Tem certeza de que deseja excluí-lo?')" data-bs-toggle="tooltip"><i class="icon-delete"></i></button>
            </th>
          </tr>
        </tbody>
      </template>
