<template>
  <form @submit="formSubmit">
    <CardTitle title="Configurações de E-mail" icon="fas fa-envelope"></CardTitle>
    <div class="row row-cols-2">
      ...
      <InputField class="col" :is-required="true" :label="$t('setting_mail_page.lbl_username')" placeholder="seuemail@gmail.com" v-model="mail_username" :error-message="errors.mail_username"></InputField>
      <InputField class="col" :is-required="true" :label="$t('setting_mail_page.lbl_password')" type="password" placeholder="Senha" v-model="mail_password" :error-message="errors.mail_password"></InputField>
      <InputField class="col" :is-required="true" :label="$t('setting_mail_page.lbl_mail')" placeholder="seuemail@gmail.com" v-model="mail_from" :error-message="errors.mail_from"></InputField>
      <InputField class="col" :is-required="true" :label="$t('setting_mail_page.lbl_from_name')" placeholder="GoPet" v-model="from_name" :error-message="errors.from_name"></InputField>
    </div>
    <div class="d-grid d-md-flex gap-3 p-3">
      <SubmitButton :IS_SUBMITED="IS_SUBMITED" v-if="isEmailVerified"></SubmitButton>
      <button type="button" class="btn btn-primary" :disabled="IS_VERIFY_SUBMITED" @click="verifyEmail">Verificar Email</button>
    </div>
  </form>
</template>
