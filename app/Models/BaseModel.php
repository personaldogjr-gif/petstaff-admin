...
class BaseModel extends Model implements HasMedia
{
    ...
    public function getStatusLabelAttribute()
    {
        switch ($this->status) {
            case '0':
                return '<span class="badge bg-soft-danger">Inativo</span>';
                break;

            case '1':
                return '<span class="badge bg-soft-success">Confirmado</span>';
                break;

            case '2':
                return '<span class="badge bg-soft-warning text-dark">Aguardando</span>';
                break;
    ...
    public function setSlugAttribute($value)
    {
        if ($this->exists && !isset($this->attributes['slug'])) {
            // The model is being updated, and the slug is not set, so don't modify it.
            return;
        }
