...
function str_slug($title, $separator = '-', $language = 'pt_BR')
{
...
function getTotalAmount($start_date='', $end_date='', $type='', $service_amount=0, $employee_id=null) {
    $amount_arr = [];

    switch ($type) {
        case 'boarding':
            if ($employee_id !== null) {
                $setting_data = \App\Models\BoarderDaycareAmount::where('user_id', $employee_id)->first();

                if ($setting_data !== null) {
                    $service_amount = $setting_data->amount;
                } else {
   ...
   $amount_data=[];
    switch ($type) {
        ...
        case 'grooming':
            ...
            case 'training':
                $duration_data = Modules\Service\Models\ServiceTrainingDurationMapping::where('id',$service_id)->first();
                $service_amount = $duration_data->amount;
                $tax_amount = TaxCalculation($service_amount);
                $total_amount = $service_amount + $tax_amount;
...
function geDaycareAmount($type='',$employee_id=null){
