...
class UserController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        Flash::success("<i class='fas fa-check'></i> Novo usuário criado")->important();
        ...
        if ($request->email_credentials == 1) {
            ...
            Flash::success(icon('fas fa-envelope').' Credenciais da conta enviadas ao usuário.')->important();
        }
    ...
    public function profileUpdate(Request $request, $id)
    {
        ...
        Flash::success('<i class="fas fa-check"></i> Usuário atualizado com sucesso!')->important();
    ...
    public function changeProfilePasswordUpdate(Request $request, $id)
    {
        ...
        Flash::success(icon()." Usuário atualizado com sucesso!")->important();
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($id == 1) {
            ...
            return redirect("app/$module_name")->with('flash_success', 'Atualiado com sucesso!');
        }
    ...
    public function destroy($id)
    {
        ...
        if (auth()->user()->id == $id || $id == 1) {
            Flash::warning("<i class='fas fa-exclamation-triangle'></i> Você não pode deletar este usuário")->important();
        ...
        flash('<i class="fas fa-check"></i> Usuário deletado com sucesso!')->success();
    ...
    public function restore($id)
    {
        ...
        flash('<i class="fas fa-check"></i> Usuário restaurado com sucesso!')->success();
    ...
    public function block($id)
    {
        ...
        if (auth()->user()->id == $id || $id == 1) {
            Flash::warning("<i class='fas fa-exclamation-triangle'></i> Você não pode bloquear este usuário")->important();
        ...
        try {
            ...
            flash('<i class="fas fa-check"></i> Usuário bloqueado com sucesso!')->success();

            return redirect()->back();
        } catch (Exception $e) {
            throw new Exception('Ocorreu um problema ao atualizar este usuário. Por favor, tente novamente.');
        }
    }
    ...
    public function unblock($id)
    {
        ...
        if (auth()->user()->id == $id || $id == 1) {
            Flash::warning("<i class='fas fa-exclamation-triangle'></i> Você não pode debloquear este usuário")->important();
        ...
        try {
            ...
            flash('<i class="fas fa-check"></i> Usuário desbloqueado com sucesso!')->success();


            return redirect()->back();
        } catch (Exception $e) {
            Log::error($th);
            flash('<i class="fas fa-check"></i> Ocorreu um problema ao atualizar este usuário. Por favor, tente novamente.!')->error();
        }
    }
    ...
    public function userProviderDestroy(Request $request)
    {
        ...
        if (! $user_provider_id > 0 || ! $user_id > 0) {
            flash('Solicitação inválida. Por favor, tente novamente.')->error();
            ...
            if ($user_id == $user_provider->user->id) {
                ...
                flash('<i class="fas fa-exclamation-triangle"></i> Desvinculado do usuário, "'.$user_provider->user->name.'"!')->success();

                return redirect()->back();
            } else {
                flash('<i class="fas fa-exclamation-triangle"></i> Solicitação rejeitada. Entre em contato com o Administrador!')->warning();
            }
        }
    ...
    public function emailConfirmationResend($id)
    {
        ...
        if ($user) {
            if ($user->email_verified_at == null) {
                ...
                flash('<i class="fas fa-check"></i> E-mail enviado! Verifique sua caixa de entrada.')->success()->important();

                return redirect()->back();
            } else {
                flash($user->name.', Você já confirmou seu endereço de e-mail em '.$user->email_verified_at->isoFormat('LL'))->success()->important();
    ...
    {
        $user = auth()->user();
        $user = User::with('boarderdaycareamount')->where('id', $user->id)->first();

        if ($user->hasRole('boarder') || $user->hasRole('day_taker')) {
            $amount = 0;
            $user['amount'] = optional($user->boarderdaycareamount)->amount;
            // Check if the user has no amount set in boarderdaycareamount
            if ($user->boarderdaycareamount === null) {
                // Determine the role and set the appropriate default amount
                if ($user->hasRole('boarder')) {
                    $defaultAmountSetting = Setting::where('name', 'pet_boarding_amount')->first();
                    if ($defaultAmountSetting) {
                        
                        $amount = $defaultAmountSetting->val;
                        
                    }
                } else if ($user->hasRole('day_taker')) {
                    $defaultAmountSetting = Setting::where('name', 'pet_daycare_amount')->first();
                    if ($defaultAmountSetting) {
