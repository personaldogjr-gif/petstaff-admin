...
class SettingController extends Controller
{
    ...
    public function verify_email(Request $request)
    {
        ...
        try {
            ...
            Mail::raw('Este é um e-mail de teste de variação de e-mail smtp!', function ($message) use ($mailObject) {
                $message->to($mailObject['email'])->subject('Test Email');
            });
            return response()->json(['message' => 'Verificação feita com sucesso', 'status' => true], 200);
        } catch (\Exception $e) {
            return response()->json(['message' => 'Verificação Falhou', 'status' => false], 500);
        }
    }
