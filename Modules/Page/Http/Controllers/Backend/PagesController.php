...
class PagesController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('page::backend.pages.index_datatable', compact('module_action', 'filter', 'create_title'));
    }
    ...
    public function index_data(Datatables $datatable, Request $request)
    {
        ...
        $datatable = $datatable->eloquent($query)
            ...
            ->editColumn('name', function ($data) {
                return "<a href='".route('pages', ['slug' => $data->type])."' target='_blank'>$data->name</a>";
            })
        ...
        return $datatable->rawColumns(array_merge(['action', 'status', 'check','name']))
            ->toJson();
    }
    ...
    public function get_page($slug){
        $data = Page::where('type',$slug)->first();
        $module_action = '';
