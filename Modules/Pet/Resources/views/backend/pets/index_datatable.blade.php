    ...
    <x-backend.section-header>
      ...
      <x-slot name="toolbar">
        <div class="input-group flex-nowrap">
          ...
          <input type="text" class="form-control form-control-sm dt-search" placeholder="Buscar..." aria-label="Buscar"
            aria-describedby="addon-wrapping">            
          
        </div>
        
      @hasPermission('add_owners')
          <x-buttons.offcanvas target='#form-offcanvas' title="{{ __('Criar') }} {{ __('pet.lbl_owner') }}" class=" d-flex align-items-center gap-1">{{ __('messages.new') }}</x-buttons.offcanvas>
        @endhasPermission
      </x-slot>
    </x-backend.section-header>
  </div>
...
<div data-render="app">
  ...
  <send-push-notification create-title="Enviar Notificação PUSH"></send-push-notification>
  <change-password create-title="Alterar Senha"></change-password>
  <assign-pet create-title="Atribuir Pet"></assign-pet>
  <pet-offcanvas
...
<div data-render="app">
     <pets-offcanvas
          create-title="{{ __('Criar') }} {{ __('pet.title') }}"
          edit-title="{{ __('Editar') }} {{ __('pet.title') }}">
    </pets-offcanvas>
 </div>
@endsection
