<template>
            ...
            <div class="col-md-12 p-0">
              <div class="col-md-12 p-0 text-center">
                ...
                <div class="d-flex align-items-center justify-content-center gap-2">
                  ...
                  <input type="button" class="btn btn-soft-danger mb-3" name="remove" value="Remover" @click="removeLogo()" v-if="ImageViewer" />
                </div>
  ...
  const reset_datatable_close_offcanvas = (res) => {
    ...
    const validationSchema = yup.object({
      ...
      mobile: yup.string()
      .required('Telefone é obrigatório').matches(/^(\+?\d+)?(\s?\d+)*$/, 'Deve conter somente dígitos'),
      password : yup.string().test('password','Senha é obrigatória' , function(value) {
      ...
      confirm_password : yup.string().test('confirm_password', 'Confirmação de senha é obrigatória', function(value) {
        if(currentId === 0 && !value){
          return false;
        }
        return true
      }
      ).oneOf([yup.ref('password')], 'As senhas devem ser corresponder'),
