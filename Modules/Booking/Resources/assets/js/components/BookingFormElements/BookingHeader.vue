<template>
  ...
  <div class="d-flex align-items-center gap-2">
    <div v-if="booking_id > 0 && status !== 'cancelled' && status !== 'check_in'">
        <strong><button data-bs-toggle="tooltip" title="Cancelar Reserva" type="button" @click="changeBookingStatus('cancelled','Tem certeza de que deseja cancelar?')" class="btn btn-sm text-danger"><i data-v-f9741b98="" class="fa-regular fa-trash-can"></i></button></strong>
    </div>
...
import {confirmcancelSwal} from '@/helpers/utilities'
...
const changeBookingStatus = (status,message) => {
  confirmcancelSwal({title: message}).then((result) => {
