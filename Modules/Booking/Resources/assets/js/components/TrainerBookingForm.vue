<template>
      ...
      <template v-else-if="SINLGE_STEP == 'MAIN' && status != 'checkout'">
        ...
        <div>
           <div class="d-flex text-center date-time d-none">
            <div class="col-6 py-3">
              <i>Em</i> <strong v-if="start_date_time && start_date_time !== 'Invalid date'">{{ moment(start_date_time).format('D, MMM YYYY') }}</strong>
              <strong v-else> {{ moment(current_date).format('D, MMM YYYY') }}</strong>
            </div>
            <div class="col-6 py-3">
              <i>A</i> <strong v-if="start_date_time && start_date_time !== 'Invalid date'">{{ moment(start_date_time).format('HH:mm') }}</strong>
        ...
        <div class="offcanvas-body border-top">
          <div class="form-group">
            ...
            <Multiselect id="user_id" v-else v-model="user_id" placeholder="Selecione o Cliente" :disabled="is_paid || filterStatus(status).is_disabled" :value="user_id" v-bind="singleSelectOption" :options="customer.options" @select="customerSelect" class="form-group"></Multiselect>
          ...
          <div v-if="selectedCustomer">
            <div class="col-md-12 form-group">
              ...
              <Multiselect id="pet" v-model="pet" :value="pet" placeholder="Selecione o Pet" v-bind="SingleSelectOption" :options="pet_list.options" class="form-group"></Multiselect>
            ...
            <div class="row">
              <div class="col-md-6">
                <div class="form-group">
                  ...
                  <flat-pickr placeholder="Data" id="date" class="form-control" v-model="date" :value="date" :config="config"></flat-pickr>
              ...
              <div class="col-md-6">
                <div class="form-group">
                  ...
                  <flat-pickr placeholder="Hora" id="time" class="form-control" v-model="time" :value="time" :config="config_time"></flat-pickr>
            ...
            <div class="col-md-12 form-group">
              ...
              <Multiselect id="employee_id" v-model="employee_id" :value="employee_id" placeholder="Selecione o Treinador" @select="getTrainingList" v-bind="SingleSelectOption" :options="trainer_list.options" class="form-group"></Multiselect>
            ...
            <div class="col-md-12 form-group">
              ...
              <Multiselect id="training" v-model="training" :value="training" placeholder="Selecione o Treinamento" @select="getDurationList" v-bind="SingleSelectOption" :options="training_list.options" class="form-group"></Multiselect>
            ...
            <div class="col-md-12 form-group">
              ...
              <Multiselect id="duration" v-model="duration" :value="duration" placeholder="Selecione a Duração" @select="checkTotalAmount" v-bind="SingleSelectOption" :options="duration_list.options" :allow-empty="false" class="form-group"></Multiselect>
  ...
  <CustomeFormOffcanvas createTitle="Criar Cliente"></CustomeFormOffcanvas>
  <PetFromOffcanvas createTitle="Criar Pet"></PetFromOffcanvas>
</template>
<script setup>
...
import { CUSTOMER_LIST, PET_LIST, TRAINER_LIST, TRAINING_LIST, TRAINER_DURATION_LIST, DURATION_PRICE,TAX_DATA } from '../constant/booking'
...
const validationSchema = yup.object({
  ...
  pet: yup.string().required('Pet é obrigatório'),
  date: yup.string().required('Data é obrigatório'),
  time: yup.string().required('Hora é obrigatório'),
  employee_id: yup.string().required('Treinador é obrigatório'),
  training: yup.string().required('Treinamento é obrigatório'),
...
const getTrainingList = (employeeId) => {
  useSelect({ url: TRAINING_LIST, data: { employee_id:employeeId } }, { value: 'id', label: 'name' }).then((data) => {
    training_list.value = data
    if (data.options.length > 0) {
      training.value = data.options[0].value
    }
    const type_id = training_list.value.list.length ? training_list.value.list[0].id : null;
    if (type_id) {
      getDurationList(type_id);
    } else {
...
const getTrainerList = () => {
  useSelect({ url: TRAINER_LIST }, { value: 'id', label: 'full_name' }).then((data) => {
    trainer_list.value = data
    if (data.options.length > 0) {
      employee_id.value = data.options[0].value
    }
    const employeeId = trainer_list.value.list.length ? trainer_list.value.list[0].id : null;
    if (employeeId) {
      getTrainingList(employeeId);
...
const getDurationList = (type_id) => {
  useSelect({ url: TRAINER_DURATION_LIST, data: { type: 'training', type_id: type_id } }, { value: 'id', label: 'duration' }).then((data) =>{
    if (data && Array.isArray(data.list)) {
        ...
        console.error('Invalid duration list data:', data);
      }
    // duration_list.value = data
    let selected_duration = data.list.find(item => item.id === duration.value);
      if (selected_duration) {
        checkTotalAmount(selected_duration.price);
      }
  })
}
...
const checkTotalAmount = (price) => {
    let total_amount = taxCalculation(price);
    totalAmount.value = total_amount;
  
};
watch(training, (value) => {
  if (value) {
    duration.value = ''; 
    totalAmount.value = 0;
    totalTaxAmount.value = 0;
...
watch(duration, (value) => {
  if (value) {
    let selected_duration = duration_list.value.list.find(item => item.id === value);
    if (selected_duration) {
      checkTotalAmount(selected_duration.price);
    }
  }
