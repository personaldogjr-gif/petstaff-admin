            ...
            <div class="row">
              ...
              <div class="col-md-3">
                <div class="form-group">
                  ...
                  <flat-pickr placeholder="Hora de retirada" id="pick_up_time" class="form-control" v-model="pick_up_time" :value="pick_up_time" :config="config_time"></flat-pickr>
              ...
              <div class="col-md-6 form-group">
                ...
                <Multiselect id="employee_id" v-model="employee_id" :value="employee_id" placeholder="Selecione o Cuidador" v-bind="SingleSelectOption" @select="checkTotalAmount"  :options="daycare_taker_list.options" class="form-group"></Multiselect>
                <span class="text-danger">{{ errors.employee_id }}</span>
              </div>
            </div>

            <div class="row">
              <InputField class="col-md-6" type="text" :label="$t('booking.lbl_favorite_activity')" placeholder="Atividade Favorita" v-model="favorite_activity" :error-message="errors['favorite_activity']" :error-messages="errorMessages['favorite_activity']"></InputField>
              <InputField class="col-md-6" type="text" :label="$t('booking.lbl_favorite_food')" placeholder="Comida Favorita" v-model="favorite_food" :error-message="errors['favorite_food']" :error-messages="errorMessages['favorite_food']"></InputField>
            </div>
...
const validationSchema = yup.object({
  ...
  pet: yup.string().required('Selecione o Pet é obrigatório'),
  date: yup.string().required('Data é obrigatório'),
  drop_off_time: yup.string().required('Hora de entrega é obrigatória'),
  pick_up_time: yup.string()
    .required('Hora de retirada é obrigatória')
    .test(
      'is-greater',
      'A data de retirada deve ser posterior à data de entrega',
      function (value) {
        const { drop_off_time } = this.parent;
        if (!drop_off_time || !value) return true; // If drop_off_time or value is not set, skip validation
        const dropOffTime = new Date(`1970-01-01T${drop_off_time}:00Z`);
        const pickUpTime = new Date(`1970-01-01T${value}:00Z`);
        return pickUpTime > dropOffTime;
      }
    )
    .test(
      'is-not-same',
      'O horário de coleta não pode ser igual ao horário de entrega',
      function (value) {
        const { drop_off_time } = this.parent;
        if (!drop_off_time || !value) return true; // If drop_off_time or value is not set, skip validation
        return value !== drop_off_time;
      }
    ),
...
const checkTotalAmount = (employee_id) => {
if(date.value !=''){

  const type = 'pet_daycare_amount'
  let pet_daycare_amount = 0
  listingRequest({ url: BOOKING_PRICE, data: { type: type, employee_id:employee_id } }).then((res) => {
    if (res.data) {
...
const getDaycareTakerList = () => {
  useSelect({ url: DAYCARE_TAKER_LIST }, { value: 'id', label: 'full_name' }).then((data) =>{
    daycare_taker_list.value = data
    const employee_id = daycare_taker_list.value.list.length ? daycare_taker_list.value.list[0].id : null;
    if (employee_id) {
      checkTotalAmount(employee_id);
    } else {
