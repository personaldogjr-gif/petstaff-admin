    ...
    public function index_data(Datatables $datatable, Request $request)
    {
        ...
        return $datatable->eloquent($query)
            ...
            ->editColumn('pet_name', function ($data) {
                return view('booking::backend.bookings.datatable.pet_id', compact('data'));
    ...
    public function store(BookingRequest $request)
    {
        ...
        switch ($booking['booking_type']) {
            case 'boarding':
                ...
                $service_amount = getTotalAmount($data['drop_off_date'], $data['pick_up_date'], $data['type'], 0, $data['employee_id']);
                ...
                $booking_mapping=[
                    ...
                    'price'=> $service_amount['total_service_amount'],
                 ];
            ...
            case 'walking':
                ...
                case 'daycare':
                    ...
                    $service_amount=geDaycareAmount($data['type'],$data['employee_id']);
    
                    $booking_mapping=[
    
                        'booking_id'=>$booking['id'],
                        'date'=>new DateTime($data['date'].''.$data['drop_off_time']),
            ...
            default:
                ...
                // For example:
                echo "Tipo de reserva não reconhecido";
                break;
        }
        ...
        try {
            ...
            $type = 'new_booking';
            $messageTemplate = 'Nova reserva #[[booking_id]] foi criada';
    ...
    public function updateStatus($id, Request $request)
    {
       ...
       $booking->update();
        ...
        switch ($status) {
            case 'confirmed':
                ...
                $messageTemplate = 'Reserva #[[booking_id]] foi confirmada.';
                $notify_message = str_replace('[[booking_id]]', $id, $messageTemplate);
                break;
            case 'rejected':
                $notify_type = 'reject_booking';
                $messageTemplate = 'Reserva #[[booking_id]] foi rejeitada.';
                $notify_message = str_replace('[[booking_id]]', $id, $messageTemplate);
                break;    
            case 'completed':
                $notify_type = 'complete_booking';
                $messageTemplate = 'Reserva #[[booking_id]] foi concluída.';
                $notify_message = str_replace('[[booking_id]]', $id, $messageTemplate);
                break;
            case 'cancelled':
                $notify_type = 'cancel_booking';
                $messageTemplate = 'Reserva #[[booking_id]] foi cancelada.';
                $notify_message = str_replace('[[booking_id]]', $id, $messageTemplate);
                break;
        }
       ...
       // dd($booking->start_date_time);
        ...
        if (isset($notify_type)) {
            try {            
            $this->sendNotificationOnBookingUpdate($notify_type,$notify_message,$notification_data);
            } catch (\Exception $e) {
                \Log::error($e->getMessage());
            }
        }
