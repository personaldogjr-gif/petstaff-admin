...
class TrainingController extends Controller
{
    ...
    public function index_list(Request $request)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            $customerName = $value->booking->user->full_name ?? 'Anônimo';
        ...
        foreach ($employees as $employee) {
            $resource[] = [
                ...
                'titleHTML' => '<div class="d-flex gap-3 justify-content-center align-items-center py-3"><img src="'.$employee->profile_image.'" class="avatar avatar-40 rounded-pill" alt="funcionario" />'.$employee->full_name.'</div>',
            ];
        }
    ...
    public function index_data(Datatables $datatable, Request $request)
    {
        ...
        return $datatable->eloquent($query)
            ...
            ->editColumn('duration', function ($data) {
                return formatTime(optional($data->training)->duration) ?? '-';
            })
