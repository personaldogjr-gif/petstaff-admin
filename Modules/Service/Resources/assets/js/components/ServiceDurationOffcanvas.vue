...
const config_minutes = ref({
  ...
  const validationSchema = yup.object({
    hours: yup.string().required('Horas são obrigatórias'),
    minutes: yup.string().required('Minutos são obrigatórios'),
    price: yup.string().required('Preço é obrigatório').matches(/^[1-9]\d*(\.\d+)?$/, 'Somente números positivos são permitidos'),
  })
