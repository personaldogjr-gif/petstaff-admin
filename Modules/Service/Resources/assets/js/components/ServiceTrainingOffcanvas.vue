    ...
    <div class="offcanvas offcanvas-end offcanvas-booking" tabindex="-1" id="form-offcanvas" aria-labelledby="form-offcanvasLabel">
      ...
      <div class="offcanvas-body">
        ...
        <div class="row">
          ...
          <div class="col-md-12 pt-0" v-for="(duration, index) in durations" :key="index">
            <div class="row">
              <div class="col-md-6">
                <div class="form-group">
                  <label class="form-label" :for="'hours-' + index">{{ $t('service.lbl_hours') }} <span class="text-danger">*</span></label>
                  <flat-pickr :placeholder="$t('service.lbl_hours')" :id="'hours-' + index" class="form-control" v-model="duration.hours" :config="config_hours"></flat-pickr>
                  <span class="text-danger">{{ duration.errors.hours }}</span>
                </div>
              </div>
              <div class="col-md-6">
                <div class="form-group">
                  <label class="form-label" :for="'minutes-' + index">{{ $t('service.lbl_minutes') }} <span class="text-danger">*</span></label>
                  <flat-pickr :placeholder="$t('service.lbl_minutes')" :id="'minutes-' + index" class="form-control" v-model="duration.minutes" :config="config_minutes"></flat-pickr>
                  <span class="text-danger">{{ duration.errors.minutes }}</span>
                </div>
              </div>
              ...
              <div class="row" v-if="durations.length > 1">
...
const defaultDuration = { hours: '00', minutes: '30', amount: '', status: 1, errors: {} }
const currentId = useModuleId(() => {
  if (currentId.value > 0) {
    getRequest({ url: EDIT_URL, id: currentId.value }).then((res) => {
      if (res.status && res.data) {
        durations.value = res.data.training_duration && res.data.training_duration.length > 0 
...
const validationSchema = yup.object({
  ...
  description: yup.string().nullable().test('no-script-tags', 'Descrição não pode conter script tags.', function (value) {
...
const formSubmit = handleSubmit((values) => {
  ...
  for (const duration of values.durations) {
    const key = `${duration.hours}:${duration.minutes}`;
...
const addDuration = () => {
  ...
  const isPositiveInteger = Number.isInteger(Number(lastDuration.amount)) && Number(lastDuration.amount) > 0;
  ...
  if (!isDuplicate && lastDuration.hours && lastDuration.minutes && isPositiveInteger) {
    durations.value.push({ hours: '', minutes: '', amount: '', status: 1, errors: {} });
...
watch(durations, (newDurations) => {
  newDurations.forEach((duration, index) => {
    watch(() => duration.hours, (newHours) => {
      if (newHours) {
        duration.errors.hours = ''
      }
    })
    watch(() => duration.minutes, (newMinutes) => {
      if (newMinutes) {
        duration.errors.minutes = ''
      }
    })
    watch(() => duration.amount, (newAmount) => {
      const isPositiveInteger = Number.isInteger(Number(newAmount)) && Number(newAmount) > 0
      if (newAmount && isPositiveInteger) {
        duration.errors.amount = ''
      } else if (!isPositiveInteger) {
        duration.errors.amount = 'Price cannot be negative or zero.'
      }
    })
  })
}, { deep: true })
