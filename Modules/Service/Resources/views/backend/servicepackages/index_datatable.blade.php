...
@section('content')
                ...
                <x-slot name="toolbar">
                  ...
                  {{-- @can('add_' . $module_name)
                        <x-buttons.offcanvas target='#form-offcanvas'
                            title="{{ __('Criar') }} {{ __($module_title) }}" class=" d-flex align-items-center gap-1">{{ __('messages.new') }}</x-buttons.offcanvas>
                    @endcan --}}
                    @hasPermission('create_service_package')
                      <button type="button" class="btn btn-primary" data-bs-toggle="tooltip" data-crud-id="{{0}}" title="{{ __('Criar') }} {{ __($module_title) }}"><i class="fas fa-plus-circle"></i>{{ __('Criar') }} {{ __($module_title) }}</button>
                    @endhasPermission
                </x-slot>
        </x-backend.section-header>
        </div>
    ...
    <div data-render="app">
        ...
        create-title="{{ __('Criar') }} {{ __($module_title) }}"
        edit-title="{{ __('Editar') }} {{ __($module_title) }}">
        </service-package-form-offcanvas>
    </div>
@endsection
