      ...
      <x-slot name="toolbar">
        <div class="input-group flex-nowrap">
          <div class="datatable-filter">
              <select name="duration_rolewise" id="duration_rolewise" class="select2 form-control"
                  data-filter="select" style="width: 100%">
                  <option value="all" >{{__('product.all')}}</option>
                  <option value="my_duration">
                      {{ __('messages.my_duration') }}</option>
  ...
  const columns = [{
    ...
    document.addEventListener('DOMContentLoaded', (event) => {
      initDatatable({
        ...
        advanceFilter: () => {
                    return {
                        duration_rolewise: $('#duration_rolewise').val(),
                    }
                }
      });
      let table = $('#datatable').DataTable();

$('#duration_rolewise').change(function() {
  const selectedValue = $(this).val();
  if (selectedValue === 'my_duration') {
