      ...
      <x-slot name="toolbar">
        <div class="input-group flex-nowrap">
          <div class="datatable-filter">
              <select name="facility_rolewise" id="facility_rolewise" class="select2 form-control"
                  data-filter="select" style="width: 100%">
                  <option value="all" >{{__('product.all')}}</option>
                  <option value="my_facility">
                      {{ __('messages.my_facility') }}</option>
        ...
        <div class="input-group flex-nowrap">     
          ...
          <x-buttons.offcanvas target='#form-offcanvas' title="{{ __('Criar') }} {{ __($module_title) }}" class="d-flex align-items-center gap-1">{{ __('messages.new') }}</x-buttons.offcanvas>
    ...
    document.addEventListener('DOMContentLoaded', (event) => {
      initDatatable({
        ...
        finalColumns,
                advanceFilter: () => {
                    return {
                        facility_rolewise: $('#facility_rolewise').val(),
                    }
                }
      });
        let table = $('#datatable').DataTable();

        $('#facility_rolewise').change(function() {
          const selectedValue = $(this).val();
          if (selectedValue === 'my_facility') {
