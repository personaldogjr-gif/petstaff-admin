...
class ServiceFacilityController extends Controller
{
    public function facilityList(Request $request)
    {
        ...
        // Se employee_id for fornecido, retornar facilidades disponíveis para esse funcionário
        if ($employee_id) {
            $servicefacility = $servicefacility->where(function ($query) use ($employee_id, $admin_ids) {
                // Incluir facilidades criadas pelo funcionário
                $query->where('created_by', $employee_id)
                      // E também facilidades criadas por admins (disponíveis para todos)
                      ->orWhereIn('created_by', $admin_ids);
            });
        }
        
        if(($added_by_admin == 1)){
            $servicefacility = $servicefacility->where(function ($subQuery) use ($employee_id, $admin_ids) {
