...
class ServicesController extends Controller
{
    ...
    public function index_data(Datatables $datatable, Request $request)
    {
        ...
        $datatable = $datatable->eloquent($query)
            ...
            ->addColumn('action', function ($data) use ($module_name, $user) {
                return view('service::backend.services.action_column', compact('module_name', 'data','user'));
            })
            ->editColumn('employee_count', function ($data) {
                return "<b>$data->employee_count</b>  <button type='button' data-assign-module='".$data->id."' data-assign-target='#service-employee-assign-form' data-assign-event='employee_assign' class='btn btn-soft-primary btn-sm rounded text-nowrap px-2 ' data-bs-toggle='tooltip' title='Atribuir funcionário ao serviço'><i class='fa-solid fa-plus p-0'></i></button>";
            })
            ...
            ->editColumn('status', function ($row) use ($user) {
                $checked = '';
                if($row->created_by == auth()->id() || $user->hasRole(['admin', 'demo_admin'])){
                if ($row->status) {
        ...
        if (! request()->is_single_branch) {
            $datatable->editColumn('branches_count', function ($data) {
                return "<b>$data->branches_count</b>  <button type='button' data-assign-module='".$data->id."' data-assign-target='#service-branch-assign-form' data-assign-event='branch_assign' class='btn btn-soft-primary btn-sm rounded text-nowrap px-2' data-bs-toggle='tooltip' title='Atribuir filial ao serviço'><i class='fa-solid fa-plus p-0'></i></button>";
            });
        }
    ...
    public function assign_employee_list($id)
    {
        ...
        $service_user = $service_user->each(function ($data) {
            $data['name'] = $data->employee->first_name.' '.$data->employee->last_name;
