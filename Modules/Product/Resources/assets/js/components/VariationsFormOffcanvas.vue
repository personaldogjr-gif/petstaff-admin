          ...
          <div class="form-group" v-if="type">
            <div v-for="(input, index) in variationValues" :key="index" class="d-flex gap-3 align-items-end">
              <div v-if="type == 'color'">
                ...
                <span class="text-danger color_value_err" v-if="input.value.value === ''">Valor é requerido</span>
              </div>
              <div v-else-if="type == 'text'">
                ...
                <span class="text-danger text_value_err" v-if="input.value.value === ''">Valor é requerido</span>
              </div>
              <div class="flex-grow w-100">
                ...
                <span class="text-danger name_err" v-if="input.value.name === ''">Nome é requerido</span>
              </div>
  ...
  const formSubmit = handleSubmit((values) => {
    ...
    if (values.type == 'color') {
      ...
      if (hasEmptyColorName) {
        document.querySelector('.color_value_err').textContent = 'Valor é requerido';
        return false;
      }
    }
    if (values.type == 'text') {
      ...
      if (hasEmptyTextValue) {
        document.querySelector('.text_value_err').textContent = 'Valor é requerido';
        return false;
      }
    }
    const hasEmptyName = values.values.some((value) => value.name === '');
    if (hasEmptyName) {
      document.querySelector('.name_err').textContent = 'Nome é requerido';
      return false;
    }
