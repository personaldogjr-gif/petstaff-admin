...
@section('content')
              ...
              <div class="d-flex flex-wrap gap-3">
                ...
                <x-slot name="toolbar">
                  ...
                  <div class="input-group flex-nowrap">
                    ...
                    <input type="text" class="form-control dt-search" placeholder="Buscar..." aria-label="Buscar" aria-describedby="addon-wrapping">
                  </div>
                    @hasPermission('create_subcategories')
                      <button type="button" class="btn btn-primary" data-crud-id="{{0}}" title="{{ __('messages.create') }} {{ __($module_title) }}"><i class="fas fa-plus-circle me-2"></i>{{ __('messages.create') }} {{ __('category.singular_title') }}</button>
                    @endhasPermission
    ...
    <div data-render="app">
        <category-form-offcanvas
            ...
            create-title="{{ __('messages.create') }}{{ __('category.new') }} {{ __('category.singular_title') }}" edit-title="{{ __('messages.edit') }}{{ __('category.sub_category') }}"
            create-nested-title="{{ __('messages.create') }}{{ __('novo') }}  Sub {{ __($module_title) }}" edit-nested-title="{{ __('messages.edit') }} Sub {{ __($module_title) }}"
            :category-id="{{ isset($category->id) ? $category->id : 0 }}"
            ></category-form-offcanvas>
    </div>
@endsection
...
@push('after-scripts')
    ...
    const columns = [
            ...
            { data: 'mainCategory.name', name: 'mainCategory.name', title: "{{ __('category.lbl_category') }}" ,width: '15%', searchable: false, orderable: false, },
