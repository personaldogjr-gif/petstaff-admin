...
class OrdersController extends Controller
{
    ...
    public function updateDeliveryStatus(Request $request)
    {
        ...
        switch ($status) {
            case 'accept':
                ...
                $messageTemplate = 'Pedido #[[order_id]] foi aceito.';
                $notify_message = str_replace('[[order_id]]', $orderItem->id, $messageTemplate);
                break;
            case 'processing':
                $notify_type = 'order_proccessing';
                $messageTemplate = 'Pedido #[[order_id]] está em processamento.';
                $notify_message = str_replace('[[order_id]]', $orderItem->id, $messageTemplate);
                break;
            case 'delivered':
                $notify_type = 'order_delivered';
                $messageTemplate = 'Pedido #[[order_id]] foi entregue.';
                $notify_message = str_replace('[[order_id]]', $orderItem->id, $messageTemplate);
                break;    
            case 'cancelled':
                $notify_type = 'order_cancelled';
                $messageTemplate = 'Pedido #[[order_id]] foi cancelado.';
                $notify_message = str_replace('[[order_id]]', $orderItem->id, $messageTemplate);
                break;
        };
        ...
        return response()->json(['status' => true, 'message' => 'Seu pedido foi atualizado com sucesso']);
    }
        ...
        if (count($carts) > 0) {
            ...
            foreach ($carts as $cart) {
                ...
                if ($cart->qty > $productVariationStock) {
                    $message = $cart->product_variation->product->name.' esta fora de estoque';
            ...
            $type = 'order_placed';
            $messageTemplate = 'Novo pedido #[[order_id]] foi feito.';
            $notify_message = str_replace('[[order_id]]', $order->id, $messageTemplate);
            $this->sendNotificationOnOrderUpdate($type,$notify_message,$notification_data);
            } catch (\Exception $e) {
                \Log::error($e->getMessage());
            }
    ...
    public function updateExpectedDeliveryDate(Request $request){
        ...
        return response()->json(['status' => true, 'message' => 'Data de entrega esperada atualizada com sucesso']);
    }
}
