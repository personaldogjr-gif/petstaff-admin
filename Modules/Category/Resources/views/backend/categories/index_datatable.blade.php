...
@section('content')
            ...
            <x-slot name="toolbar">
              <div>
                ...
                @hasPermission('add_category')
                  <button type="button" title="{{ __('Criar') }} Categoria / Subcategoria" class="btn btn-primary d-flex align-items-center gap-1" data-crud-id="{{0}}"><i class="icon-add-new"></i>{{ __('messages.new') }}</button>
                @endhasPermission
            </x-slot>
    ...
    <div data-render="app">
      <form-offcanvas
              ...
              create-nested-title="{{ __('Criar') }} {{ __('novo') }} Sub {{ __($module_title) }}" edit-nested-title="{{ __('Editar') }} Sub {{ __($module_title) }}"
