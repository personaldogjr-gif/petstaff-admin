...
class EmployeesController extends Controller
{
    ...
    public function index_data(Datatables $datatable, Request $request)
    {
        ...
        $datatable = $datatable->eloquent($query)
            ...
            ->editColumn('one_day_price', function ($data) {
                $boarderdaycare = BoarderDaycareAmount::where('user_id', $data->id)->first();
         
                if($boarderdaycare !== null) {
                    $value = $boarderdaycare->amount;
                } else {
                    if($data->user_type == 'boarder'){
                        $data = Setting::where('name', 'pet_boarding_amount')->first();
                    }elseif($data->user_type == 'day_taker'){
                        $data = Setting::where('name', 'pet_daycare_amount')->first();
                    }
                    
                    
                    if($data !== null) {
                        $value =  (double)$data->val;
                    } else {
                        $value = 0; // Handle the case when no settings are found
                    }
                }
                return $value;
                // return '<span class="badge booking-status bg-soft-primary p-3">'.str_replace("_","",ucfirst($data->user_type)).'</span>';
