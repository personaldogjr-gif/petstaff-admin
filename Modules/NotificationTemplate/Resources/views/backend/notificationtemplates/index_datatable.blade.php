      ...
      <x-backend.section-header>
            ...
            <x-slot name="toolbar">
              ...
              {{-- @hasPermission('add_notification_template')
                <a href='{{ route("backend.notification-templates.create") }}' class="btn btn-primary d-flex align-items-center gap-1" title="{{__('Criar')}}  {{ __($module_title) }}"><i class="icon-add-new"></i>{{__('Criar novo')}}</a>
              @endhasPermission --}}
            </x-slot>
        </x-backend.section-header>
    </div>
